...
- **minimal-CDP stealth mode** to clear full-page Cloudflare **Managed Challenges** (CAS-217, experimental, env-gated). The "Just a moment…" interstitial is decided by CDP/automation detection, not rendering/GPU/profile/clicks: real headful + real-GPU WebGL + warmed profile + humanized/real clicks all stay walled, while a *clean* CDP browser (nodriver) auto-passes in ~4s. chromiumoxide hardwires the tells into its per-target init chain — `Runtime.enable` (→ `Runtime.consoleAPICalled`), `Network.enable`, `Performance.enable`, `Log.enable`, `Target.setAutoAttach(waitForDebuggerOnStart)`, and an isolated-world `addScriptToEvaluateOnNewDocument`. Vendored chromiumoxide 0.9.1 via `[patch.crates-io]` skips all of them when `VOIDCRAWL_STEALTH_NO_RUNTIME=1`; VoidCrawl then enables only Page (+ on-demand Accessibility/DOM/Input, main-world `Runtime.evaluate`) and **auto-passes the Managed Challenge (verified 3/3)**. `eval_js` still works. Trade-offs in this mode: no network capture / network-idle goto, no cross-origin `evaluate_js_in_frame`, no `evaluate_function`, no OOPIF auto-attach. Default behavior unchanged. See `docs/stealth.md`.
