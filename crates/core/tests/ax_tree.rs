...
async fn click_by_role_clicks_the_named_element() {
    ...
    page.click_by_role("button", "Subscribe", 0, false).await.expect("click_by_role failed");
...
async fn click_by_role_disambiguates_with_nth() {
    ...
    page.click_by_role("button", "Go", 1, false).await.expect("click_by_role nth=1 failed");
