...
async def part1_self_contained(tab) -> None:
    ...
    # Let the script-attached child frame register.
    ...
    secret = await tab.evaluate_js_in_frame(
...
async def part2_live_recaptcha(tab) -> None:
    print("\n=== Part 2: live reCAPTCHA bframe (cross-origin, real site) ===")
    # A third-party page that embeds Google reCAPTCHA. The reCAPTCHA iframes are
    # served from google.com → cross-origin to the host page. This is exactly
    # the wall that blocks same-origin captcha solvers on real sites.
    url = "https://2captcha.com/demo/recaptcha-v2"
    try:
        await tab.goto(url, timeout=30.0)
    except Exception as exc:
        print(f"  (skipped — could not load {url}: {exc})")
        return
    ...
    read_checkbox = (
        ...
        " ' aria-checked=' + c.getAttribute('aria-checked')) : 'no checkbox yet'; })()"
    )
    inside = None
    for _ in range(20):
        try:
            inside = await tab.evaluate_js_in_frame(
                "recaptcha/api2/anchor", read_checkbox
            )
            break
        except Exception:
            await asyncio.sleep(0.5)
    ...
    from_parent = await tab.evaluate_js(
        "(() => { const f = document.querySelector('iframe[src*=\\\"api2/anchor\\\"]');"
        " return f && f.contentDocument ? 'READABLE' : 'NULL (cross-origin)'; })()"
    )
    print(f"  parent  sees anchor frame DOM → {from_parent!r}")
    if inside is None:
        print(
            "  (reCAPTCHA anchor frame never appeared — site layout may have changed)"
        )
        return
...
async def main() -> None:
    async with BrowserPool(PoolConfig()) as pool, pool.acquire() as tab:
        ...
        await part2_live_recaptcha(tab)
